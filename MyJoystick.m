...
-(id) init {
  ...
  if ( (self = [super init]) ) {
    ...
    leftJoy.joystick = [[SneakyJoystick alloc] initWithRect:CGRectMake(0,0,96,96)];
    leftJoystick = [leftJoy.joystick autorelease];
...
//    jButton.position = ccp(winSize.width - (jButton.contentSize.width) - margin, jButton.contentSize.height + margin);
...
//    aButton.position = ccp(winSize.width - (aButton.contentSize.width) - margin, aButton.contentSize.height + margin + 100);
